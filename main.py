...
def create_new_user(user: PyUser):
    ...
    if username_validator(user.username_api):
        # password input validation
        if password_validator(user.password_api):

            # search user table for existing username
            user_search = session.query(UserModel).filter(
                UserModel.username == user.username_api).first()

            if user_search is None:
                try:
                    # hash input password
                    user_hashed_password = bcrypt.hash(user.password_api)

                    # create new user or object
                    new_user = UserModel(username=user.username_api,
...
def generate_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    """
    # username input validation
    if username_validator(form_data.username):
        # password input validation
        if password_validator(form_data.password):
            ...
            if user is not None:
                ...
                if bcrypt.verify(form_data.password, user.hashed_password):
...
def list_of_countries(offset_val: int = Query(None, description="please enter offset value"),
    ...
    if isinstance(offset_val, int) and isinstance(limit_val, int):
        try:
            ...
            countries_list = session.query(CountryModel).offset(
...
def find_country_by_city_name(city: str = Query(None, description="Please enter city name")):
    """
    ...
    """
    if city.isalpha():
        city_data = session.query(CityModel).filter(
...
def list_of_cities(offset_val: int = Query(None, description="Please enter offset value"),
    ...
    if isinstance(offset_val, int) and isinstance(limit_val, int):
        try:
            cities_list = session.query(CityModel).offset(
...
def list_cities_by_population(min: int = Query(None,
                              ...
                              max: int = Query(None,
                                               description="Please enter maximum value")):
    """
    ...
    if isinstance(min, int) and isinstance(max, int):
        try:
            search_cities = session.query(CityModel).filter(
...
def nearest_and_farthest_cities(country_id,
    ...
    if country_id.isdigit():
        ...
        country_obj = session.query(CountryModel).filter(
...
def three_nearest_cities(country, token: str = Depends(oauth2_scheme)):
    """
    Task B
...
@app.get("/nswe_cities/")
def nswe_cities(country_name_input, token: str = Depends(oauth2_scheme)):
    """
    Task C
...
def ls_cities(value_set,
    ...
    if country_set_validator(value_set):
        ...
        country_list = [session.query(CountryModel).filter(
            func.lower(CountryModel.country_name) == func.lower(country)).first()
